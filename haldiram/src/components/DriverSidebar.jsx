...
export default function VendorSidebar() {
  ...
  // make logout robust and async; also close mobile overlay when logging out
  async function handleLogout(closeMobile = false) {
    try {
      if (closeMobile) setMobileOpen(false);
      // if your logout returns a promise, await it
      if (logout && typeof logout === "function") {
        await logout();
      } else {
        // defensive fallback: clear common auth keys
      ...
      // navigate to login regardless (ensures UI moves away)
      try {
        navigate("/login");
      } catch {}
    }
