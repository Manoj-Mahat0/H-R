...
function AddProductDrawer({ product, onClose, onSuccess }) {
  ...
  async function handleSubmit(e) {
    ...
    if (!form.product_code.trim()) {
      toast("Product code is required", "error");
      return;
    } 
  ...
  const tabs = [
    ...
    { id: 'db', label: 'DB Metrics', icon: FiDatabase },
    { id: 'rss', label: 'RSS Metrics', icon: FiShoppingBag },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4">
      ...
      {/* Main Container - Responsive height and width */}
      <div className="relative bg-white dark:bg-gray-800 w-full sm:rounded-2xl shadow-2xl sm:max-w-6xl flex flex-col max-h-[100vh] sm:max-h-[90vh]">
        
        {/* Header - Fixed */}
        <div className="flex-shrink-0 px-4 sm:px-6 py-4 sm:py-5 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-gray-800 dark:to-gray-900 sm:rounded-t-2xl">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-gray-100 flex items-center gap-2">
                <FiPackage className="text-indigo-600 dark:text-indigo-400 w-5 h-5 sm:w-6 sm:h-6" />
          ...
          <div className="flex gap-1 sm:gap-2 overflow-x-auto no-scrollbar -mx-4 px-4 sm:mx-0 sm:px-0">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                type="button"
                ...
                className={`
                  flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg sm:rounded-xl font-medium text-xs sm:text-sm whitespace-nowrap transition-all
                  ${activeTab === tab.id
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto">
          <div className="p-4 sm:p-6 space-y-6">
            ...
            {activeTab === 'pricing' && (
              <div className="space-y-6 animate-fadeIn">
                <div className="bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 rounded-2xl p-4 sm:p-6 border border-emerald-200 dark:border-emerald-800">
                  <h4 className="text-base sm:text-lg font-bold text-emerald-900 dark:text-emerald-100 mb-4 flex items-center gap-2">
                ...
                {/* Cost Calculations */}
                <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-2xl p-4 sm:p-6 border border-blue-200 dark:border-blue-800">
                  <h4 className="text-base sm:text-lg font-bold text-blue-900 dark:text-blue-100 mb-4 flex items-center gap-2">
            ...
            {activeTab === 'db' && (
              <div className="space-y-6 animate-fadeIn">
                <div className="bg-gradient-to-br from-violet-50 to-purple-50 dark:from-violet-900/20 dark:to-purple-900/20 rounded-2xl p-4 sm:p-6 border border-violet-200 dark:border-violet-800">
                  <h4 className="text-base sm:text-lg font-bold text-violet-900 dark:text-violet-100 mb-4 flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  <MetricCard
                    label="DB/Pcs (With GST)"
                    value={`₹${calculated.dbPerPcsWithGST}`}
                    color="violet"
                    icon={FiTrendingUp}
                    tooltip="DB per Pcs (Basic) + GST"
                  />
                  <MetricCard
                    label="DB/CB (Basic)"
                    ...
                    color="violet"
                    icon={FiBox}
                    tooltip="DB per Pcs × Pcs per CB"
                  />
                  <MetricCard
                    label="DB/CB (With GST)"
                    value={`₹${calculated.dbPerCBWithGST}`}
                    color="purple"
                    icon={FiDollarSign}
                    tooltip="DB per CB + GST"
                    highlight
                  />
                </div>
              </div>
            ...
            {activeTab === 'rss' && (
              <div className="space-y-6 animate-fadeIn">
                <div className="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 rounded-2xl p-4 sm:p-6 border border-amber-200 dark:border-amber-800">
                  <h4 className="text-base sm:text-lg font-bold text-amber-900 dark:text-amber-100 mb-4 flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  <MetricCard
                    label="RSS/Pcs (With GST)"
                    value={`₹${calculated.rssPerPcsWithGST}`}
                    color="amber"
                    icon={FiTrendingUp}
                    tooltip="RSS per Pcs (Basic) + GST"
                  />
                  <MetricCard
                    label="RSS/CB (Basic)"
                    ...
                    color="amber"
                    icon={FiBox}
                    tooltip="RSS per Pcs × Pcs per CB"
                  />
                  <MetricCard
                    label="RSS/CB (With GST)"
                    value={`₹${calculated.rssPerCBWithGST}`}
                    color="orange"
                    icon={FiDollarSign}
                    tooltip="RSS per CB + GST"
                    highlight
                  />
                </div>
              </div>
            ...
            {activeTab === 'analytics' && (
              <div className="space-y-6 animate-fadeIn">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  <MarginCard
                    label="DB Margin"
                    value={`${calculated.marginDB}%`}
                    formula="((DB - Cost) / Cost) × 100"
                    color="emerald"
                    icon={FiTrendingUp}
                  />
                  <MarginCard
                    label="RSS Margin"
                    value={`${calculated.marginRSS}%`}
                    formula="((RSS - Cost) / Cost) × 100"
                    color="blue"
                    icon={FiTrendingUp}
                  />
                  <MarginCard
                    label="Margin Difference"
                    value={`${calculated.marginDiff}%`}
                    formula="DB Margin - RSS Margin"
                    color="purple"
                    icon={FiActivity}
                    highlight
                  />
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead className="bg-gray-50 dark:bg-gray-800">
                        <tr>
                          <th className="px-4 py-3 text-left font-semibold text-gray-900 dark:text-gray-100">Metric</th>
                          <th className="px-4 py-3 text-right font-semibold text-violet-700 dark:text-violet-400">DB</th>
                          <th className="px-4 py-3 text-right font-semibold text-amber-700 dark:text-amber-400">RSS</th>
                          <th className="px-4 py-3 text-right font-semibold text-gray-700 dark:text-gray-300">Diff</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                        <ComparisonRow
                          ...
                          diff={`₹${((parseFloat(form.db_per_pcs_basic) || 0) - (parseFloat(form.rss_per_pcs_basic) || 0)).toFixed(2)}`}
                        />
                        <ComparisonRow
                          label="Per Pcs (GST)"
                          db={`₹${calculated.dbPerPcsWithGST}`}
                          rss={`₹${calculated.rssPerPcsWithGST}`}
                          diff={`₹${(parseFloat(calculated.dbPerPcsWithGST) - parseFloat(calculated.rssPerPcsWithGST)).toFixed(2)}`}
                        />
                        <ComparisonRow
                          label="Per CB (Basic)"
                          db={`₹${calculated.dbPerCBBasic}`}
                          rss={`₹${calculated.rssPerCBBasic}`}
                          diff={`₹${(parseFloat(calculated.dbPerCBBasic) - parseFloat(calculated.rssPerCBBasic)).toFixed(2)}`}
                        />
                        <ComparisonRow
                          label="Per CB (GST)"
                          db={`₹${calculated.dbPerCBWithGST}`}
                          rss={`₹${calculated.rssPerCBWithGST}`}
                          diff={`₹${(parseFloat(calculated.dbPerCBWithGST) - parseFloat(calculated.rssPerCBWithGST)).toFixed(2)}`}
                          highlight
                        />
                      </tbody>
                    </table>
        ...
        {/* Footer - Fixed */}
        <div className="flex-shrink-0 px-4 sm:px-6 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 sm:rounded-b-2xl">
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
            <div className="text-xs text-gray-500 dark:text-gray-400 hidden sm:block">
              <span className="flex items-center gap-1">
                <FiInfo className="w-3.5 h-3.5" />
                All calculations are auto-updated
              </span>
            </div>
            <div className="flex gap-3">
              <button
                type="button"
                onClick={onClose}
                className="flex-1 sm:flex-none px-6 py-2.5 rounded-xl border-2 border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all font-medium"
                disabled={saving}
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 sm:flex-none px-6 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-bold hover:from-indigo-700 hover:to-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-indigo-500/50 flex items-center justify-center gap-2"
                disabled={saving}
              >
                {saving ? (
                  <>
...
function InputFieldModern({ label, icon: Icon, className = "", ...props }) {
  return (
...
function ComparisonRow({ label, db, rss, diff, highlight }) {
  return (
    <tr className={highlight ? 'bg-indigo-50 dark:bg-indigo-900/20 font-semibold' : ''}>
      <td className="px-4 py-3 text-gray-900 dark:text-gray-100">{label}</td>
      <td className="px-4 py-3 text-right text-violet-700 dark:text-violet-400">{db}</td>
      <td className="px-4 py-3 text-right text-amber-700 dark:text-amber-400">{rss}</td>
      <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-300">{diff}</td>
    </tr>
  );
}
...
/* ---------- Read Only Field Component - ENHANCED UI/UX - FIXED CLOSING BRACES ---------- */
function ReadOnlyField({ label, value, tooltip, highlight = false, color }) {
    const colorClasses = {
        blue: { bg: 'bg-blue-50 dark:bg-blue-900/20', border: 'border-blue-300 dark:border-blue-600', text: 'text-blue-800 dark:text-blue-200' },
        emerald: { bg: 'bg-emerald-50 dark:bg-emerald-900/20', border: 'border-emerald-300 dark:border-emerald-600', text: 'text-emerald-800 dark:text-emerald-200' },
        amber: { bg: 'bg-amber-50 dark:bg-amber-900/20', border: 'border-amber-300 dark:border-amber-600', text: 'text-amber-800 dark:text-amber-200' },
    }[color] || {};

  return (
    <div>
      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5 flex items-center gap-1" title={tooltip}>
        {label}
        {tooltip && (
          <FiAlertCircle className="w-4 h-4 text-gray-400 dark:text-gray-500 cursor-help" />
        )}
      </label>
      <div className={`w-full px-4 py-2.5 border rounded-xl font-mono text-lg ${
        highlight 
          ? `${colorClasses.bg} ${colorClasses.border} ${colorClasses.text} font-extrabold border-2` 
...
function StockModal({ product, stockData, loading, onClose, onRefresh }) {
  ...
  async function handleAddStock(e) {
    ...
    try {
      setAddingStock(true);
      // Fix the API request to properly send JSON data
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      {/* Increased max-w to 6xl for side-by-side view */}
      <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={onClose} />
      <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full **max-w-6xl** transform transition-all duration-300 scale-100">
        <div className="px-6 py-5 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
          <h3 className="text-xl font-bold text-gray-900 dark:text-gray-100 flex items-center gap-2">
            <FiDatabase className="text-indigo-600 dark:text-indigo-400 w-5 h-5" />
            Stock Entries for: {product.product_name}
          </h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="p-6 **grid grid-cols-1 lg:grid-cols-2 gap-6**">
          ...
          {/* Add Stock Form - Left Column, enhanced color */}
          <div className="p-6 **bg-indigo-50 dark:bg-indigo-900/20** rounded-2xl border border-indigo-200 dark:border-indigo-700 shadow-lg h-fit">
            <h4 className="text-xl font-bold **text-indigo-700 dark:text-indigo-300** mb-4 flex items-center gap-2">
                <FiPlus className="w-5 h-5" /> Add New Stock Entry
            </h4>
            <form onSubmit={handleAddStock} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border rounded-xl border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setNewStock({...newStock, manufacturing_date: e.target.value})}
                  className="w-full px-3 py-2 border rounded-xl border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100"
                  required
                />
              </div>
              ...
              <div className="md:col-span-2 flex justify-end">
                <button
                  ...
                  className="px-6 py-2.5 **bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 disabled:opacity-50 transition-colors shadow-md shadow-indigo-500/50**"
                >
          ...
          {/* Stock List - Right Column, enhanced style */}
          <div className="p-4 bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-lg">
            <h4 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-4 flex items-center justify-between">
              Current Stock Entries
              <button
                onClick={onRefresh}
                disabled={loading}
                className="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl bg-blue-100 dark:bg-blue-900/30 text-xs font-semibold text-blue-700 dark:text-blue-300 hover:bg-blue-200 dark:hover:bg-blue-800/40 transition-colors disabled:opacity-50"
                title="Refresh Stock"
              >
                <FiRefreshCw className={loading ? "animate-spin w-3 h-3" : "w-3 h-3"} />
                Refresh
              </button>
            </h4>
            {loading ? (
              <div className="text-center py-4 text-gray-500 dark:text-gray-400">
                <FiRefreshCw className="inline animate-spin mr-2 w-5 h-5" /> Loading stock data...
              </div>
            ) : stockData.length === 0 ? (
              <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                <FiDatabase className="mx-auto w-12 h-12 mb-3 text-gray-300 dark:text-gray-600" />
                <p>No stock entries found for this product</p>
              </div>
            ) : (
              <div className="space-y-3 max-h-[28rem] overflow-y-auto pr-2">
                {stockData.map((stock) => (
                  <div key={stock.id} className="p-4 border rounded-xl border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 hover:shadow-lg transition-shadow duration-300">
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <div className="text-sm text-gray-600 dark:text-gray-300">
                          Mfd: <span className="font-semibold">{new Date(stock.manufacturing_date).toLocaleDateString()}</span>
                        </div>
                      </div>
                      <div className="text-right">
                        {/* Highlight Expiry Date in Red */}
                        <div className="text-xs text-gray-500 dark:text-gray-400">
                          Exp: <span className="font-semibold **text-red-500**">{new Date(stock.expire_date).toLocaleDateString()}</span>
                        </div>
                        <div className="text-xs text-gray-500 dark:text-gray-400">
                          Entry: {new Date(stock.date_of_entry).toLocaleDateString()}
                        </div>
                      </div>
                    </div>
                    {stock.notes && (
                      <div className="text-xs mt-2 p-2 bg-gray-100 dark:bg-gray-600 rounded-lg text-gray-600 dark:text-gray-300">
                        <span className="font-bold">Note:</span> {stock.notes}
                      </div>
                    )}
                  </div>
                ))}
