    ...
    <div className="min-h-screen flex bg-gray-50">
      ...
      <main className="flex-1 p-6 md:p-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={openAdd}
              className="inline-flex items-center gap-2 px-3 py-2 rounded bg-indigo-600 text-white hover:bg-indigo-700 text-sm"
            >
      ...
      {openForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <form
            onSubmit={handleSave}
            className="relative z-10 bg-white rounded-lg shadow-xl w-full max-w-2xl p-6"
          >
            <div className="flex items-start justify-between">
              <div>
                <h3 className="text-lg font-semibold">{editing ? "Edit vehicle" : "Add vehicle"}</h3>
                <div className="text-xs text-gray-500 mt-1">{editing ? `ID: ${editing.id}` : "Create a new vehicle"}</div>
              </div>
              <button type="button" onClick={closeForm} className="text-gray-500">Close</button>
            </div>

            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <label className="block text-xs text-gray-700">Vehicle number</label>
                <input value={form.vehicle_number} onChange={(e) => setForm(f => ({...f, vehicle_number: e.target.value}))} className="mt-1 w-full px-3 py-2 border rounded" />
              </div>

              <div>
                <label className="block text-xs text-gray-700">Driver mobile</label>
                <input value={form.driver_mobile} onChange={(e) => setForm(f => ({...f, driver_mobile: e.target.value}))} className="mt-1 w-full px-3 py-2 border rounded" />
              </div>

              <div>
                <label className="block text-xs text-gray-700">Latitude</label>
                <input value={form.lat} onChange={(e) => setForm(f => ({...f, lat: e.target.value}))} className="mt-1 w-full px-3 py-2 border rounded" />
              </div>

              <div>
                <label className="block text-xs text-gray-700">Longitude</label>
                <input value={form.lng} onChange={(e) => setForm(f => ({...f, lng: e.target.value}))} className="mt-1 w-full px-3 py-2 border rounded" />
              </div>

              <div>
                <label className="block text-xs text-gray-700">Capacity weight</label>
                <input value={form.capacity_weight} onChange={(e) => setForm(f => ({...f, capacity_weight: e.target.value}))} className="mt-1 w-full px-3 py-2 border rounded" />
              </div>

              <div>
                <label className="block text-xs text-gray-700">Capacity unit</label>
                <select value={form.capacity_unit} onChange={(e) => setForm(f => ({...f, capacity_unit: e.target.value}))} className="mt-1 w-full px-3 py-2 border rounded">
              ...
              <div className="md:col-span-2">
                <label className="block text-xs text-gray-700">Details</label>
                <input value={form.details} onChange={(e) => setForm(f => ({...f, details: e.target.value}))} className="mt-1 w-full px-3 py-2 border rounded" />
              </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative z-10 bg-white rounded-lg shadow-xl w-full max-w-sm p-6">
            <h3 className="text-lg font-semibold">Delete vehicle?</h3>
            <p className="text-sm text-gray-600 mt-2">This will deactivate/delete the vehicle. Are you sure?</p>
            <div className="mt-6 flex justify-end gap-3">
              <button onClick={() => setDeleteTarget(null)} className="px-4 py-2 rounded border">Cancel</button>
              <button onClick={handleConfirmDelete} disabled={deleting} className="px-4 py-2 rounded bg-red-600 text-white">
