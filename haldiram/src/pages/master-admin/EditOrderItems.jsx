...
export default function EditOrderItems() {
    ...
    async function submitEdit({ closeOnSuccess = true } = {}) {
        ...
        try {
            ...
            // Refresh order data
            try {
                const orderData = await apiRequest(`/orders/${order.id}`);
                setOrder(orderData);
            } catch (err) {
    ...
    async function confirmOrderWithVehicle() {
        ...
        try {
            ...
            toast(`Order ${order.id} confirmed with vehicle ${selectedVehicle.vehicle_number}`, "success");
            navigate(-1);
        } catch (err) {
            ...
            <main className="flex-1 p-8">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-8">
                    <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
                        <div className="flex items-start gap-4">
                            <button
                                onClick={() => navigate(-1)}
                                className="flex items-center gap-2 px-4 py-2 bg-gray-50 hover:bg-gray-100 text-gray-700 rounded-xl border border-gray-200 transition-all duration-200"
                            >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-[calc(100vh-12rem)]">
                        ...
                        <div className="flex-1 overflow-y-auto pr-2">
                            <div className="space-y-4">
                                {filteredProducts.map((p) => {
                                    ...
                                    return (
                                        <div key={pid} className="border border-gray-200 rounded-xl p-4 bg-gradient-to-r from-gray-50 to-white hover:shadow-md transition-all duration-200">
                                            <div className="flex items-center justify-between gap-3 mb-3">
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <button
                                                        onClick={async () => {
                                                            // Use the robust helper instead of setTimeout fragile logic
                                            ...
                                            <button
                                                ...
                                                className="w-full flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors duration-150"
                                            >
                                            ...
                                            {isExpanded && (
                                                <div className="mt-4 space-y-3">
                                                    {batches === null ? (
                                                        ...
                                                        batches.map((b) => (
                                                            <div key={b.id ?? b.batch_no ?? `${pid}-${Math.random()}`} className="bg-white rounded-lg p-3 border border-gray-100">
                                                                <div className="flex items-center justify-between mb-2">
                                                                    <div className="font-semibold text-gray-900">
                                                                        Batch: {b.batch_no || "N/A"}
                                                                    </div>
                                                                    <div className="px-2 py-1 bg-blue-50 text-blue-700 rounded-full text-sm font-semibold">
                                                                        Qty: {b.quantity ?? "N/A"}
                                                                    </div>
                                                                </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 overflow-hidden flex flex-col">
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-500">{editRows.length} items • Weight: {orderWeight} units</p>
                                </div>
                            </div>
                            <button
                                onClick={addEmptyRow}
                        ...
                        <div className="flex-1 overflow-y-auto pr-2">
                            {editErrors?.global && (
                                <div className="p-3 rounded-lg bg-red-50 border border-red-100 text-red-700 mb-4">{editErrors.global}</div>
                            )}
                            ...
                            <div className="space-y-4 mb-6">
                                {editRows.map((row, idx) => {
                                    ...
                                    return (
                                        <div key={row.id ?? row.tempId ?? idx} className="bg-gray-50 border border-gray-200 rounded-xl p-4">
                                            <div className="grid grid-cols-12 gap-3 items-center">
                                                ...
                                                <div className="col-span-4 md:col-span-2">
                                                    ...
                                                    <input
                                                        ...
                                                        className={`mt-1 w-full px-3 py-2 border rounded-lg text-sm ${editErrors[`row_qty_${idx}`] ? "border-red-500" : "border-gray-200"}`}
                                                    />
                ...
                <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-8 py-4 z-50">
                    ...
                    {editSaving && (
                        <div className="mb-3">
                            ...
                            <div className="mt-1 text-xs text-gray-500">Saving changes — {Math.round(editSaveProgress || 0)}%</div>
                        </div>
                    )}
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-6">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="font-medium">Items: <span className="text-gray-900">{editRows.length}</span></span>
                            </div>
                            <div className="flex items-center gap-2">
                                <FiPackage className="w-4 h-4 text-gray-500" />
                                <span className="font-medium">Total Qty: <span className="text-gray-900">{editRows.reduce((s, r) => s + Number(r.qty || 0), 0)}</span></span>
                            </div>
                        ...
                        <div className="flex items-center gap-3">
                            <input
                                ...
                                className="px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm w-64"
                            />

                            <button
                                onClick={() => navigate(-1)}
                                disabled={editSaving}
                                className="flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-xl text-gray-700 hover:bg-white hover:shadow-sm transition-all duration-150"
                            >
